...
function transformApiDataToFormData(apiData) {
  ...
  formData._tem_deficiencia = !!(
    apiData.deficiencias &&
    apiData.deficiencias.length > 0 &&
    apiData.deficiencias[0] !== "Não"
  );
  ...
  formData.problemas_renais = !!(
    apiData.doencas_renais &&
    apiData.doencas_renais.length > 0 &&
    apiData.doencas_renais[0] !== "Não tem"
  );
  formData._doenca_respiratoria_tipo =
    apiData.doencas_respiratorias?.[0] || "Não tem";
  if (apiData.condicao_rua) {
    ...
    formData._possui_referencia_familiar =
      !!apiData.condicao_rua.referencia_familiar;
    if (apiData.condicao_rua.refeicoes_dia === 1)
      formData._refeicoes_ao_dia = "Uma";
    else if (apiData.condicao_rua.refeicoes_dia === 2)
      formData._refeicoes_ao_dia = "Duas";
    else if (apiData.condicao_rua.refeicoes_dia === 3)
      formData._refeicoes_ao_dia = "Três";
    else if (
      apiData.condicao_rua.refeicoes_dia &&
      apiData.condicao_rua.refeicoes_dia > 3
    )
      formData._refeicoes_ao_dia = "Mais de três";
    else formData._refeicoes_ao_dia = "";
    formData._visita_familiar = !!apiData.condicao_rua.visita_familiar;
    formData._grau_parentesco_visita =
      apiData.condicao_rua.graus_parentesco_familiar || "";
    formData._acompanhado_por_instituicao =
      !!apiData.condicao_rua.instituicao_apoio;
    formData._nome_instituicao = apiData.condicao_rua.instituicao_apoio || "";
    const origens = apiData.condicao_rua.origem_alimentacao || [];
    formData._origem_alimentacao_restaurante_popular = origens.includes(
      "Restaurante Popular"
    );
    formData._origem_alimentacao_doacao_grupo_religioso = origens.includes(
      "Doação Grupo Religioso"
    );
    formData._origem_alimentacao_doacao_restaurante =
      origens.includes("Doação Restaurante");
    formData._origem_alimentacao_doacao_popular =
      origens.includes("Doação Popular");
    ...
    formData._acesso_higiene_sanitario = acessos.includes(
      "Acesso ao sanitário"
    );
...
function UpdateCidadao() {
  ...
  useEffect(() => {
    if (cidadao) {
      ...
      console.log(
        "Cidadão não encontrado ou erro ao buscar, limpando formulário."
      );
      ...
      {!loading &&
        (!transformedData ? (
          <div className="loader">
            {cidadao === null
              ? "Nenhum cidadão encontrado com este ID."
              : "Aguardando dados..."}
          </div>
        ) : (
          <div>
